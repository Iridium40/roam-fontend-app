...
const calendarProviders: CalendarProvider[] = [
  {
    ...
    features: ['Two-way sync', 'Event creation', 'Conflict detection', 'Automatic reminders']
  },
  {
    type: 'outlook',
    ...
    features: ['Office 365 sync', 'Meeting scheduling', 'Team calendar access', 'Exchange integration']
  },
  {
    type: 'apple',
    name: 'Apple Calendar',
    ...
    features: ['iCloud sync', 'Cross-device access', 'Native iOS integration', 'Family calendar sharing']
  }
];

export const CalendarIntegration: React.FC<CalendarIntegrationProps> = ({ providerId }) => {
  const { user } = useAuth();
  ...
  const handleSyncNow = async (connectionId: string, calendarType: CalendarType) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/60">
            Sync your bookings with your calendar to avoid conflicts and stay organized.
          </p>
        </div>
        ...
        <Dialog open={isSetupDialogOpen} onOpenChange={setIsSetupDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Choose a calendar provider to sync your bookings and availability.
              </DialogDescription>
            </DialogHeader>
            
            <div className="space-y-4">
              {calendarProviders.map((provider) => {
                const isConnected = connections.some(c => c.calendar_type === provider.type);
                
                return (
                  <Card 
                    key={provider.type}
                    className={`cursor-pointer transition-all ${
                      selectedProvider?.type === provider.type ? 'ring-2 ring-roam-blue' : 'hover:shadow-md'
                  ...
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between">
                        ...
                        {selectedProvider?.type === provider.type && !isConnected && (
                          <CheckCircle className="w-5 h-5 text-roam-blue" />
                        )}
                      </div>
                      
                      <div className="mt-3">
                        <p className="text-xs font-medium text-foreground/70 mb-2">Features:</p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleSyncNow(connection.id, connection.calendar_type)}
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-sm text-foreground/60 mb-4">
              Connect your calendar to automatically sync bookings and prevent double-booking.
            </p>
            <Button 
          ...
          <Alert>
            ...
            <AlertDescription>
              ...
              <ul className="mt-2 space-y-1 text-sm">
                <li>• Bookings are automatically added to your connected calendars</li>
